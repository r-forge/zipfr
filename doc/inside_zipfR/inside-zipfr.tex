...
\label{sec:app:overlap}

\begin{itemize}
\item one important application is the comparison of two samples $A$ and $B$ from populations containing the same types, though possibly with different frequency distributions; for convenience, we also refer to the populations as $A$ and $B$
\item formally, we assume a joint vocabulary $\onetoS$ for the two populations; type $i$ has occurrence probability $\pi_i$ in $A$ and $\pi'_i$ in $B$, each of which may be zero (if a type doesn't occur in this population); types are listed by descending sum of the probabilities $\pi_i + \pi'_i$, which may be different from the canonical ordering for either population; importantly, $i$ always refers to \emph{the same} type in both populations
\item type frequency counts are given by r.v.\ $f_i$ for $A$ and $f_i'$ for $B$; the individual frequency spectra of the samples are denoted by $V, V_m$ for $A$ and $V', V'_m$ for $B$; and the sample sizes are $N, N'$
\item the key statistic for analysing vocabulary overlap is the \textbf{joint frequency spectrum} (jfs)
  \[
    V_{k,k'} = \sumi \IV{f_i = k \wedge f'_i = k'}
  \]
  which takes the form of a two-dimensional table
  \[
    \begin{array}{c|ccccccc}
      & V'_0 & V'_1 & V'_2 & V'_3 & V'_4 & V'_5 & \ldots\\
      \hline
      V_0 & V_{0,0} & V_{0,1} & V_{0,2} & V_{0,3} & V_{0,4} & V_{0,5} & \ldots \\
      V_1 & V_{1,0} & V_{1,1} & V_{1,2} & V_{1,3} & V_{1,4} & V_{1,5} & \ldots \\
      V_2 & V_{2,0} & V_{2,1} & V_{2,2} & V_{2,3} & V_{2,4} & V_{2,5} & \ldots \\
      V_3 & V_{3,0} & V_{3,1} & V_{3,2} & V_{3,3} & V_{3,4} & V_{3,5} & \ldots \\
      V_4 & V_{4,0} & V_{4,1} & V_{4,2} & V_{4,3} & V_{4,4} & V_{4,5} & \ldots \\
      V_5 & V_{5,0} & V_{5,1} & V_{5,2} & V_{5,3} & V_{5,4} & V_{5,5} & \ldots \\
      \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \ddots
    \end{array}
  \]
\item the jfs is usually much sparser than the spectra of individual samples, which makes the usual approach based on normal approximations unsuitable, except for the very low end of the jfs and/or very large samples
\item the marginal spectra are obtained by summing over the rows and columns of the jfs:
  \[
    V_m = \sum_{k'} V_{m,k'} \qquad
    V'_{m'} = \sum_{k} V_{k,m'}
  \]
  and the marginal vocabulary sizes by summing over everything except for the first row or first column, respectively
  \[
    V = \sum_{k=1}^{\infty} \sum_{k'} V_{k,k'} \qquad
    V' = \sum_{k} \sum_{k'=1}^{\infty} V_{k,k'}
  \]
  
\item the vocabulary size and frequency spectrum of the union $A\cup B$ of both samples are obtained by summing over diagonals:
  \begin{align*}
    \Vcup &= \sum_{k+k' > 0} V_{k,k'} = S - V_{0,0} \\
    \Vcup_m &= \sum_{k'=0}^m V_{m - k', k'}
  \end{align*}
\item the overlap vocabulary size $\Vcap$ is obtained by summing over everything except for the first row and column
  \[
    \Vcap = \sum_{k, k' = 1}^{\infty} V_{k, k'}
  \]
  and can also be broken down by total frequency $m = f_i + f'_i$ of each shared type, which requires summing over shortened diagonals (excluding $V_{m,0}$ and $V_{0,m}$)
  \[
    \Vcap_m = \sum_{k'=1}^{m-1} V_{m - k', k'}
  \]
\item the number of types unique to $A$ is the sum over the first column (except for $V_{0,0}$), and analogously for the number of types unique to $B$
  \[
    \VdiffA = \sum_{k=1}^{\infty} V_{k,0} \qquad
    \VdiffB = \sum_{k'=1}^{\infty} V_{0,k'}
  \]
\item this results in several simple identities
  \begin{align*}
    \Vcup &= V + \VdiffA = V' + \VdiffB \\
    \Vcap &= V - \VdiffA = V' - \VdiffB = V + V' - \Vcup \\
    \Vcap_m &= \Vcup_m - V_{0,m} - V_{m,0}
  \end{align*}
\item finally, the shared token count of $A$ and $B$, i.e.\ the total number of tokens for the shared types, can be computed as
  \[
    N^{A\cap B} = \sum_{m=1}^{\infty} m \Vcap_m
  \]
\end{itemize}

\subsubsection{Two samples from the same population}
\label{sec:app:overlap:same}

\begin{itemize}
\item the only relatively easy case is when $A$ an $B$ are two independent samples from the same population
\item as usual, we assume independent Poisson sampling in order to simplify equations and to have a better chance of being able to work out some variances at least
\item the Poisson probability of type $i$ occurring $k$ times in a sample of size $N$ is
  \[ 
    \Exp{\IV{f_i =k}} = \p{f_i = k} = e^{-N\pi_i} \frac{(N\pi_i)^{k}}{k!}
  \]
\item since $A$ and $B$ are independent samples, the joint probability is
  \begin{align*}
    \Exp{\IV{f_i = k \wedge f'_i = k'}}
    &= \p{f_i = k \wedge f'_i = k'} \\
    &= e^{-N\pi_i} \frac{(N\pi_i)^{k}}{k!} \cdot e^{-N'\pi_i} \frac{(N'\pi_i)^{k'}}{k'!} \\
    &= e^{-(N + N') \pi_i} \frac{N^k (N')^{k'}}{k! k'!} \pi_i^{k + k'} \\
    &= e^{-(N + N') \pi_i} \frac{\bigl( (N + N') \pi_i \bigr)^{k + k'}}{(k + k')!}\cdot
      \binom{k+k'}{k}\cdot  \frac{N^k (N')^{k'}}{(N + N')^{k + k'}} \\
    &= e^{-(N + N') \pi_i} \frac{\bigl( (N + N') \pi_i \bigr)^{k + k'}}{(k + k')!}\cdot
      \binomP{k}{k + k'}{\tfrac{N}{N + N'}}
  \end{align*}
  with the binomial probability
  \[
    \binomP{k}{n}{p} = \binom{n}{k} p^k (1 - p)^{n - k}
  \]
\item the intuition behind this result is simple: the spectrum element $\Vcup_m$ represents types with a total frequency of $m = k + k'$ in the union of the samples; it is the sum over a diagonal in the jfs; types are assigned to the individual jfs cells depending on how their $m$ occurrences are split across the two samples, which leads to a binomial distribution  
\item for identical sample sizes $N = N'$ the equation simplifies further:
  \[
    \Exp{\IV{f_i = k \wedge f'_i = k'}}
    = e^{-2N \pi_i} \frac{(2N \pi_i)^{k + k'}}{(k + k')!}\cdot \binomP{k}{k + k'}{\tfrac12}
  \]
\item we can easily obtain expected spectrum sizes by summing over the probabilities
  \[
    \Exp{V_m} = \sumi \Exp{\IV{f_i = m}} 
  \]
  and correspondingly for the jfs
  \[
    \Exp{V_{k, k'}} = \sumi \Exp{\IV{f_i = k \wedge f'_i = k'}} 
  \]
\item for a practical implementation, the sum must be approximated by a Poisson integral over the type density function $g(\pi)$:
  \[
    \Exp{V_m} = \int_0^{\infty} \frac{(N\pi)^m}{m!} e^{-N\pi} g(\pi) \dpi
  \]
\item after the transformation above, the expectation of the jfs also takes the form of a Poisson integral for the union of the samples (with frequency class $m = k + k'$ and sample size $N + N'$):
  \begin{align*}
    \Exp{V_{k, k'}}
    &= \int_0^{\infty} \frac{\bigl( (N + N') \pi \bigr)^{k + k'}}{(k + k')!} e^{-(N + N')\pi}
      \cdot \binomP{k}{k + k'}{\tfrac{N}{N + N'}} \cdot g(\pi) \dpi \\
    &= \binomP{k}{k + k'}{\tfrac{N}{N + N'}}\cdot \Exp{V_{k+k'}(N + N')}
  \end{align*}
\item expected vocabulary size is computed analogously via the identity $\IV{f_i > 0} = 1 - \IV{f_i = 0}$
  \[
    \Exp{V} = \sumi \Exp{\IV{f_i > 0}} = \sumi \Exp{1 - \IV{f_i > 0}}
    = \sumi \bigl( 1 - e^{-N \pi_i} \bigr)
  \]
  and approximated via the integral
  \[
    \Exp{V} = \int_0^{\infty} \bigl( 1 - e^{-N\pi} \bigr) g(\pi) \dpi
  \]
\item we can compute expected vocabulary sizes for the union of the samples and their vocabulary differences (types unique to one sample) in a similar way
  \begin{align*}
    \Exp{\Vcup}
    &= \sumi \Exp{\IV{f_i > 0\vee f'_i > 0}} = \sumi \Exp{1 - \IV{f_i = 0\wedge f'_i = 0}}
      = \sumi \bigl( 1 - e^{-N \pi_i} e^{-N' \pi_i} \bigr) \\
    &= \sumi \bigl( 1 - e^{-(N + N') \pi_i} \bigr) = \Exp{V(N + N')} \\
    \Exp{\VdiffA}
    &= \sumi \Exp{\IV{f_i > 0\wedge f'_i = 0}}
      = \sumi \bigl( 1 - e^{-N \pi_i} \bigr) e^{-N' \pi_i} \\
    &= \sumi \bigl( 1 - e^{-(N + N') \pi_i} \bigr) - \bigl( 1 - e^{-N' \pi_i} \bigr)
      = \Exp{V(N + N')} - \Exp{V(N')} \\
    \Exp{\VdiffB}
    &= \Exp{V(N + N')} - \Exp{V(N)} 
  \end{align*}
\item based on these results, we obtain the expected vocabulary overlap as
  \begin{align*}
    \Exp{\Vcap(N, N')}
    &= \sumi \Exp{\IV{f_i > 0\wedge f'_i > 0}}
      = \sumi \bigl( 1 - e^{-N \pi_i} \bigr) \cdot \bigl( 1 - e^{-N' \pi_i} \bigr) \\
    & = \sumi \bigl( 1 - e^{-N \pi_i} \bigr) - \sumi \bigl( 1 - e^{-N \pi_i} \bigr) e^{-N' \pi_i}
      = \Exp{V(N)} - \Exp{\VdiffA} \\
    &= \Exp{V(N)} - \bigl( \Exp{V(N + N')} - \Exp{V(N')} \bigr) \\
     &= \Exp{V(N)} + \Exp{V(N')} - \Exp{V(N + N')}
  \end{align*}
\end{itemize}

\todo[inline]{re-derive these results with the easy intuitive reasoning of a joint random sample of size $N = N + N'$}
\todo[inline]{simple equation for expected frequency spectrum of overlap?}
\todo[inline]{explain why variances and the full sampling distributions are difficult and will first have to be explored via simulation experiments}

\subsubsection{Samples from different populations}
\label{sec:app:overlap:different}

\begin{itemize}
\item two populations with shared vocabulary are described by pairs of probabilities $(\pi_i, \pi'_i)$ for corresponding types, and approximated by a \textbf{joint type density function} (jtdf) $g(\pi, \pi')$
\item the baseline case of two populations with uncorrelated type probabilties
  \[
    g(\pi, \pi') = g(\pi)\cdot g(\pi')
  \]
  is extremely unrealistic and might only serve as a purely mathematical point of reference (e.g.\ \emph{the} might then be the most common word in $A$ but extremely rare in $B$); as a consequence, any realistic model will have to assume a relatively high level of correlation between the frequency distributions
\item the other extreme is the case of two identical populations $A$ and $B$ (or, equivalently, samples from the same population, as discussed in Sec.~\ref{sec:app:overlap:same}); in this case, the type mass is concentrated along the diagonal, resulting in some improper delta function similar to
  \[
    g(\pi, \pi') = g(\pi) \delta(\pi' - \pi)
  \]
\item this case therefore needs to be analysed separately (Sec.~\ref{sec:app:overlap:same}); however, this observation also means that very strongly correlated populations cannot easily be represented by a jtdf, which would be concentrated on a small area close to the diagonal and hence be poorly conditioned
\end{itemize}
