...
\todo[inline]{work out expectation $\Exp{\Vcup(N + N')}$ for the joint sample, which should be possible in terms of $\Exp{V}$ and $\Exp{V_m}$, but likely with a slightly complicated equation}
\todo[inline]{work out $\Exp{\Vcap(N, N')}$ both with the easy intuitive reasoning of a joint random sample of size $N = N + N'$ and through the connection to the expected joint frequency spectrum}
