...
\item the key statistic for analysing vocabulary overlap is the \textbf{joint frequency spectrum} (jfs)
  \[
    V_{k,k'} = \sumi \I{f_i = k \wedge f'_i = k'}
  \]
...
\todo[inline]{work out expectations $\Exp{V_{m,m'}(N, N')}$, which should be possible in terms of $\Exp{V}$ and $\Exp{V_m}$, but likely with a slightly complicated equation}
